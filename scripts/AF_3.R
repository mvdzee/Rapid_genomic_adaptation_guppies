# Use this script to calculate the number of fixed allele frequency differences between GH and each of the introduced sites
# as well as the mean/median absolute allele frequency changes and their min/max
library(data.table)
...
# create empty df
af_mat<-data.frame(matrix(ncol=7,nrow = 4))  # nrow = the nr of pairwise comparisons
...
af_mat$Population<-c("ILL","IUL","IC","IT"")
...
af_mat$`mean |dAF|`<-c(round(mean(ghll2$absDAF),digits=3),round(mean(ghul2$absDAF),digits=3),round(mean(ghc2$absDAF),digits=3),
                       round(mean(ght2$absDAF),digits=3))
af_mat$`median |dAF|`<-c(round(median(ghll2$absDAF),digits=3),round(median(ghul2$absDAF),digits=3),round(median(ghc2$absDAF),digits=3),
                         round(median(ght2$absDAF),digits=3))
...
af_mat$`min-max`<-c(ll_min,ul_min,c_min,t_min)
...
af_mat$`Unchanged MAF`<-c(nrow(ll_same),nrow(ul_same),nrow(c_same),nrow(t_same))
...
af_mat$`GHP MAF/fixed LP`<-c(ll_min_fix,ul_min_fix,c_min_fix,t_min_fix)
af_mat$`Fixed GHP/LP`<-c(ll_fixed,ul_fixed,c_fixed,t_fixed)
